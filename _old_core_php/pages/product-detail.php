...
$pageTitle = "Product Details - Fashion Store";
        ...
        <div class="container mx-auto px-6 py-4">
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2 text-sm">
                    <li>
                         <a href="<?php echo BASE_URL; ?>index.php" class="text-secondary transition-colors duration-300 flex items-center">
                    ...
                    <li class="flex items-center" aria-current="page">
                        ...
                        <span class="ml-2 text-secondary"><?php echo $product['name']; ?></span>
                    </li>
                </ol>
            </nav>
        </div>
        ...
        <div class="flex flex-col lg:flex-row">
                ...
                <div class="block md:block lg:hidden product-carousel bg-primary p-4 rounded-lg">
                    <div class="relative overflow-hidden rounded-lg">
                            ...
                            <?php foreach ($product['images'] as $index => $image): ?>
                                <div class="carousel-image w-full flex-shrink-0">
                                    <img src="<?php echo $image; ?>" alt="Product image <?php echo $index + 1; ?>"
                                        ...
                                        onclick="openFullscreen('<?php echo $image; ?>')">
                                </div>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                            <?php foreach ($product['images'] as $index => $image): ?>
                                <button
                                    class="carousel-indicator w-3 h-3 rounded-full bg-secondary bg-opacity-50 transition-all duration-300 <?php echo $index === 0 ? 'active opacity-100' : ''; ?>"
                ...
                <div class="hidden lg:block bg-primary ">
                    ...
                    <!-- <div class="mb-4">
                        <img id="mainImage" src="<?php echo $product['images'][0]; ?>"
                            ...
                            class="w-full h-96 object-cover rounded-lg cursor-pointer"
                            onclick="openFullscreen('<?php echo $product['images'][0]; ?>')">
                    </div> -->
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-2">
                        <?php foreach ($product['images'] as $index => $image): ?>
                            <div class="cursor-pointer">
                                <img src="<?php echo $image; ?>" alt="Product image <?php echo $index + 1; ?>"
                                    class="w-full h-full object-cover" onclick="openFullscreen('<?php echo $image; ?>')">
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div class="w-full md:w-1/2 lg:w-2/5 ">
                <div class="bg-primary rounded-lg p-6 sticky top-4 text-center">
                    <!-- Product Name -->
                    <h1 class="text-2xl font-bold text-secondary mb-2 text-center"><?php echo $product['name']; ?></h1>

                    <!-- Variant -->
                    <p class="text-accent text-lg mb-4 text-center"><?php echo $product['variant']; ?></p>

                    <!-- Price -->
                    <div class="flex items-center justify-center space-x-3 mb-4 ">
                        <span
                            class="text-secondary text-xl font-bold"><?php echo $product['discounted_price']; ?></span>
                        <span class="text-accent text-sm line-through"><?php echo $product['original_price']; ?></span>
                        ...
                        <div class="flex flex-wrap gap-2 justify-center">
                            <?php foreach ($product['sizes'] as $size): ?>
                                <label class="inline-flex items-center">
                                    <input type="radio" name="size" value="<?php echo $size; ?>" class="hidden peer">
                                    <span
    ...
    <div id="sizeModal" class="fixed inset-0 bg-black bg-opacity-70 z-50 hidden flex items-center justify-center p-4">
        <div class="bg-primary rounded-lg max-w-md w-full p-6">
            ...
            <div class="grid grid-cols-3 gap-3 mb-6">
                <?php foreach ($product['sizes'] as $size): ?>
                    <label class="inline-flex items-center">
                        <input type="radio" name="modal-size" value="<?php echo $size; ?>" class="hidden peer">
                        <span
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        addToCartBtn?.addEventListener('click', function () {
            ...
            if (!selectedSize) {
                ...
                // Add to cart logic here (you can add AJAX call to save to session/database)
                const quantity = parseInt(document.querySelector('.quantity-display').textContent);
