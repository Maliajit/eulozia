...
class ProductService
{
     ...
     */
    public function transformProductForListing(Product $product): array
    {
        $defaultVariant = $product->variants->where('is_default', true)->first();
        $mainImage = 'images/placeholder-product.jpg';
        $images = [];

        // Get main image and all images for carousel
        if ($defaultVariant && $defaultVariant->images && $defaultVariant->images->isNotEmpty()) {
            ...
            $images = $defaultVariant->images->map(function ($img) {
                return [
                    'url' => $img->url,
        ...
        return [
            ...
            'images' => $images, // Added for carousel
...
'variantAttributes.attributeValue:id,value,label,color_code'
            ...
            if (is_numeric($slug)) {
                $product = $query->where('id', $slug)->firstOrFail();
            } else {
