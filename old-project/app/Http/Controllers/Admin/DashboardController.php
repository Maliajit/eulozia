 ...
 * 
 * Purpose: Central hub for business intelligence and store overview metrics.
 * 
 * Data Flow: 
 * - Aggregates data from multiple tables (Orders, Products, Customers, Carts).
 * - Calculates percentage changes for key metrics compared to previous periods.
 * - Provides JSON endpoints for dynamic charts (Sales by Category, Abandoned Carts).
 * 
 * Database: 
 * - `orders`, `order_items`: Revenue and sales volume.
 * - `products`, `product_variants`: Stock status and catalog size.
 * - `customers`: User growth metrics.
 * - `carts`: Abandoned cart tracking.
 * - `categories`: Top category performance.
 * 
 * Dependencies: Carbon (Date handling), DB Facade (Raw queries for performance).
 */
