 ...
 * 
 * Purpose: Interactive dashboard for managing the store's category hierarchy.
 * 
 * Data Flow: 
 * - CRUD: Uses AJAX/Fetch for all operations (Create, Read, Update, Delete).
 * - UI: Renders listing via Tabulator for high performance and features (export, sorting).
 * - Modal: Uses a single modal for both adding and editing categories.
 * 
 * Database: 
 * - `categories`: Central table for hierarchy and organization.
 * 
 * Dependencies: Tabulator, Axios, FontAwesome, Media Library.
 */
@endphp
...
@push('scripts')
    <script>
        ...
        const axiosInstance = axios.create({
            baseURL: '{{ url('') }}/admin', // Changed from api/admin since we use admin.php routes
            headers: {
        ...
        function initializeCategoriesTable(data = []) {
            categoriesTable = new Tabulator("#categoriesTable", {
                ...
                paginationCounter: "rows",
                ajaxURL: "{{ url('') }}/admin/categories/data", // Changed to /data endpoint
                ajaxParams: {
                    sort: 'sort_order',
                    direction: 'asc'
                },
    ...
    responsive: 0
        ...
        async function loadCategoriesData(page = 1, perPage = 10) {
            try {
                const response = await axiosInstance.get('/categories/data', { // Changed to /data
                    params: {
                        page: page,
                        per_page: perPage,
                        sort: 'sort_order',
        ...
        async function toggleCategoryStatus(id, isActive) {
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axiosInstance.post(`/categories/${id}/toggle-status`, {
                        status: isActive ? 1 : 0
                    });
