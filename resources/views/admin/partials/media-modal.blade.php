...
<script>
    ...
    async function loadMedia(page = 1, search = '') {
        ...
        try {
            ...
            renderMediaGrid(response.data.data.data); // Access the inner data array
    ...
    function renderMediaGrid(media) {
        ...
        grid.innerHTML = media.map(item => {
            ...
            // Ensure file_path is passed safely
            const filePath = item.file_path ? item.file_path.replace(/\\/g, '\\\\') : '';
            return `
    ...
    function renderPagination(data) {
        ...
        // Current Page / Total
        html += `<span class="px-3 py-1 text-sm text-gray-700">Page ${data.current_page} of ${data.last_page}</span>`;

        // Next
        if (data.next_page_url) {
            html += `<button onclick="loadMedia(${data.current_page + 1})" class="px-3 py-1 bg-white border border-gray-300 rounded-md hover:bg-gray-50 text-sm">Next</button>`;
        }
    ...
    function handleGridClick(element) {
        ...
        toggleImageSelection(id, url, filePath);
    }

    function toggleImageSelection(id, url, filePath) {
        if (currentMode === 'main') {
            selectedImages = [{ id, url, file_path: filePath }];
        } else {
