 ...
 * 
 * Purpose: Comprehensive interface for creating new simple or configurable products.
 * 
 * Data Flow: 
 * - Input: Basic info, SEO, Pricing, Shipping, and Organization.
 * - Dynamic: Fetches category-specific specifications and attributes via AJAX.
 * - Media: Interfaces with `media-modal.blade.php` for image selection.
 * - Submission: POST to `admin.products.store`.
 * 
 * Database: 
 * - Stores to `products`, `product_variants`, `product_specifications`, and `product_images`.
 * 
 * Dependencies: Axios, Swal2, Select2/Custom Selects, Media Modal.
 */
@endphp
    ...
    async function loadMedia(page = 1, search = '') {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                renderMediaGrid(response.data.data.data);
