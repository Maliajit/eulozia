...
<div class="account-container max-w-7xl mx-auto px-4 py-8 mt-16">
    <div class="flex justify-between items-center mb-8">
        ...
        <button onclick="openAddressModal()" class="bg-white text-black px-6 py-3 rounded-lg font-bold hover:bg-gray-200 transition-colors">
                ...
                <div class="address-card bg-black border border-gray-800 rounded-lg p-6 relative group">
                    ...
                    <div class="flex items-center mb-4">
                        <div class="w-10 h-10 bg-gray-900 rounded-full flex items-center justify-center mr-3">
                            <i class="fas fa-{{ $address->type == 'shipping' ? 'truck' : ($address->type == 'billing' ? 'file-invoice-dollar' : 'map-marker-alt') }} text-gray-400"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-bold">{{ $address->name }}</h4>
                            <span class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{{ $address->type }}</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1 mb-6">
                        ...
                        <p class="text-gray-400 text-sm mt-2"><i class="fas fa-phone mr-1 text-gray-600"></i> {{ $address->mobile }}</p>
                    </div>

                    <div class="flex items-center gap-4 pt-4 border-t border-gray-900">
                        <button onclick="editAddress({{ json_encode($address) }})" class="text-gray-400 hover:text-white text-sm font-medium transition-colors">Edit</button>
                        
                        <form action="{{ route('customer.account.addresses.delete', $address->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this address?')">
                            @csrf
                            @method('DELETE')
                        ...
                        @if(!$address->is_default)
                            <form action="{{ route('customer.account.addresses.set-default', $address->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="text-accent hover:text-white text-sm font-medium transition-colors ml-auto">Set Default</button>
                            </form>
                        @endif
                    </div>
                </div>
    ...
    @else
        <div class="bg-black border border-gray-800 rounded-lg p-12 text-center mt-8">
            ...
            <p class="text-gray-400 mb-8 max-w-md mx-auto">Add your shipping and billing addresses to make your checkout process faster and easier.</p>
            <button onclick="openAddressModal()" class="bg-white text-black px-8 py-3 rounded-lg font-bold hover:bg-gray-200 transition-colors">
...
<!-- Address Modal -->
<div id="addressModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-gray-900 border border-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form id="addressForm" method="POST" action="{{ route('customer.account.addresses.store') }}" class="p-6 space-y-6">
            @csrf
            ...
            <div class="flex justify-end gap-4 pt-6 border-t border-gray-800">
                <button type="button" onclick="closeAddressModal()" class="px-6 py-3 rounded-lg text-white hover:bg-gray-800 transition-colors">
                    Cancel
                </button>
