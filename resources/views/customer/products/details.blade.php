            ...
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2 text-sm">
                    ...
                    <li class="flex items-center">
                        ...
                        <a href="{{ route('products.index', ['category' => $product['category']['slug']]) }}" class="ml-2 text-secondary transition-colors duration-300">{{ $product['category']['name'] }}</a>
                    </li>
                    @endif
            ...
            <div class="w-full md:w-1/2 lg:w-2/5 ">
                <div class="bg-primary rounded-lg p-6 sticky top-4 text-center">
                    ...
                    <div class="flex items-center justify-center space-x-3 mb-4 ">
                        ...
                        <span class="text-accent text-sm line-through">₹{{ number_format($product['compare_price'], 2) }}</span>
                        ...
                        <div class="flex flex-wrap gap-2 justify-center mb-4">
                            @foreach ($attributeGroup['options'] as $option)
                                <label class="inline-flex items-center">
                                    <input type="radio" name="attribute_{{ $attributeName }}" value="{{ $option['value'] }}" class="hidden peer">
                                    <span
            ...
            <div class="grid grid-cols-3 gap-3 mb-6">
                @if(isset($product['attribute_groups']) && count($product['attribute_groups']) > 0)
                    @foreach($product['attribute_groups'] as $attributeName => $attributeGroup)
                        @foreach ($attributeGroup['options'] as $option)
                            <label class="inline-flex items-center">
                                <input type="radio" name="modal-attribute-{{ $attributeName }}" value="{{ $option['value'] }}" class="hidden peer">
                                <span
