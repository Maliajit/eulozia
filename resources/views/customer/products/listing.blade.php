        ...
        <div class="container mx-auto px-6 py-4">
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2 text-sm">
                    <li>
                        <a href="{{ route('customer.home') }}" class="text-secondary transition-colors duration-300 flex items-center">
            ...
            <div class="hidden lg:block lg:w-1/4">
                <div class="bg-primary rounded-lg p-6 sticky top-4">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="{{ route('products.index') }}" id="clearAllFilters" class="text-accent text-sm hover:text-secondary transition-colors duration-300">Clear All</a>
                    </div>
                            ...
                            <div class="filter-content">
                                <div class="space-y-2">
                                    ...
                                    <label class="flex items-center">
                                        <input type="checkbox" name="category_id[]" value="{{ $cat['id'] }}" {{ isset($categoryId) && in_array($cat['id'], (array)$categoryId) ? 'checked' : '' }} class="filter-checkbox rounded bg-gray-800 border-gray-700 text-accent focus:ring-accent">
                                        <span class="ml-2 text-accent text-sm">{{ $cat['name'] }} ({{ $cat['count'] }})</span>
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="filter-content">
                                <div class="mb-4">
                                    ...
                                    <div class="flex items-center space-x-2 mb-2">
                                        <input type="number" name="min_price" value="{{ $currentMin }}" placeholder="Min" class="w-1/2 bg-gray-800 border-gray-700 text-secondary text-xs rounded px-2 py-1 focus:ring-accent">
                                        <span class="text-accent text-xs">-</span>
                                        <input type="number" name="max_price" value="{{ $currentMax }}" placeholder="Max" class="w-1/2 bg-gray-800 border-gray-700 text-secondary text-xs rounded px-2 py-1 focus:ring-accent">
                                    </div>
                                    <button type="submit" class="w-full bg-accent text-primary text-xs py-1 rounded hover:bg-gray-300 transition-colors font-bold uppercase">Apply Filters</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="filter-section">
                            ...
                            <div class="filter-content">
                                <div class="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                                    ...
                                    <label class="flex items-center">
                                        <input type="checkbox" name="brand_id[]" value="{{ $brand['id'] }}" {{ isset($brandId) && (is_array($brandId) ? in_array($brand['id'], $brandId) : $brandId == $brand['id']) ? 'checked' : '' }} class="filter-checkbox rounded bg-gray-800 border-gray-700 text-accent focus:ring-accent">
                                        <span class="ml-2 text-accent text-sm">{{ $brand['name'] }} ({{ $brand['count'] }})</span>
                                    </label>
                                    @endforeach
                                </div>
                                <button type="submit" class="w-full bg-accent text-primary text-xs py-1 mt-4 rounded hover:bg-gray-300 transition-colors font-bold uppercase">Apply Filters</button>
                            </div>
                        </div>
                        @endif
                ...
                <div class="bg-primary rounded-lg p-6 mb-6 hidden lg:block">
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            ...
                            <p class="text-accent">Showing <span id="productCount">{{ count($products) }}</span> of {{ $paginator['total'] ?? 0 }} products</p>
                        </div>
                    ...
                    <div class="mt-12 flex justify-center">
                        <nav class="flex items-center space-x-2">
                            @if($paginator['current_page'] > 1)
                                <a href="?page={{ $paginator['current_page'] - 1 }}{{ request()->filled('search') ? '&search='.request('search') : '' }}{{ request()->filled('category_id') ? '&category_id='.request('category_id') : '' }}" class="p-2 rounded-lg bg-primary text-secondary hover:bg-accent hover:text-primary transition-colors duration-300">
